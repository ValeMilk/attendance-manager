...
async function run() {
    ...
    for (const source of sourceRows) {
        ...
        if (existing) {
            await AttendanceRecord.updateOne({ _id: existing._id }, {
                $set: {
        ...
        await AttendanceRecord.updateOne({ _id: source._id }, {
            $set: {
    ...
    await AttendanceRecord.updateMany({ employeeId: TARGET_ID }, { $set: { employeeName: TARGET_NAME, supervisorId: TARGET_SUPERVISOR_ID } });
    ...
    await Employee.findOneAndUpdate({ supervisorId: TARGET_SUPERVISOR_ID, slug: slugify(TARGET_NAME) }, { $set: { name: TARGET_NAME, displayName: TARGET_NAME } }, { upsert: true });
