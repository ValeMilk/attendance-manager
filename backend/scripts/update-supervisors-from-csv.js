...
async function run() {
    ...
    for (const supName of Object.keys(grouped)) {
        ...
        if (!sup) {
            ...
            if (sup.supervisorId !== rodneySupervisorId) {
                sup.supervisorId = rodneySupervisorId;
                changed = true;
            }
            ...
            if (changed) {
                await sup.save();
                updated++;
                console.log(`Updated supervisor: ${supName}`);
            }
        }
