...
async function run() {
  ...
  for (const c of candidates) console.log({ _id: c._id, supervisorId: c.supervisorId, slug: c.slug, name: c.name });
  ...
  const canonical = await Employee.findOneAndUpdate(
    { supervisorId: 'global', slug: globalSlug },
    { $set: { name: globalName, displayName: globalName } },
