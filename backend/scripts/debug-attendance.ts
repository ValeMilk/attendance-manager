async function run() {
  ...
  if (!Array.isArray(body)) { console.log('Attendance response not array:', body); return; }
  console.log('Total records:', body.length);
  const filtered = body.filter((r: any) => r.day === day || (r.employeeId && r.employeeId.toLowerCase().includes('max')) );
  console.log('Records for day or name containing "max":', filtered.length);
  if (filtered.length > 0) console.log(JSON.stringify(filtered.slice(0,50), null, 2));
}
