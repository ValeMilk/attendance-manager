...
// AUTO SEED: Populate employees if empty
async function autoSeedEmployees() {
  try {
    ...
    if (employeeCount > 0) {
      console.log(`✅ ${employeeCount} funcionários já existem no banco.`);
      return;
    }

    console.log('🌱 Auto-populando funcionários do CSV...');

    // Try multiple paths
    const possiblePaths = [
      '/app/public/Pasta1.csv',
      '/opt/attendance-manager/frontend/public/Pasta1.csv',
      process.cwd() + '/public/Pasta1.csv'
    ];
    ...
    if (!csvPath) {
      console.warn('⚠️ CSV não encontrado nas posições:', possiblePaths);
      return;
    }
    ...
    console.log(`✅ Auto-seed concluído: ${created} novos + ${total} total funcionários`);
  } catch (error) {
...
async function main() {
  try {
    ...
    if (mongoose.connection.readyState === 1) {
      console.log('🌱 Iniciando auto-seed de funcionários...');
